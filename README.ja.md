...
- NuGet パッケージには `schema/nuget-resolution-guard.schema.json` と `sbom/ResolutionGuard.NuGet.spdx.json` も同梱していますが、これらはパッケージ完全性とオフライン参照のためです。エディター連携の正規入口は公開スキーマ URL とします。
